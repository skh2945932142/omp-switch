...
- The visual language is **"Quiet Instrument"**: untinted zinc neutrals; `--signal` teal marks *only* selection, focus, menu highlight, and switch state — primary buttons use the `--invert-*` ink/paper inversion, never teal; status is a colored dot plus quiet text; separation comes from tone and `--shadow-*`, borders only as hairlines inside lists. Dark theme follows `prefers-color-scheme` (no manual toggle yet); mono is Cascadia Mono.
