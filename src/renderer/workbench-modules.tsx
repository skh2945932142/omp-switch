...
function ModuleHeading({ eyebrow, title, count, children }: { eyebrow: string; title: string; count?: number; children?: ReactNode }): ReactElement {
  return <div className="workspace-heading module-heading"><div><span className="eyebrow">{eyebrow}</span><h1>{title}{typeof count === "number" ? <span className="heading-count">{count}</span> : null}</h1></div><div className="heading-actions">{children}</div></div>;
}
...
export function SurfaceModule({ api, profileId, kind, readOnly, onNotice }: CommonProps & { kind: "prompt" | "skill" }): ReactElement {
  ...
  return <section className="module-view module-shell">
    <ModuleHeading eyebrow={profileId} title={label} count={entries.length}>
