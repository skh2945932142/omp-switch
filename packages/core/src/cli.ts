...
export type JsonCliCommand = "list" | "get" | "validate" | "apply" | "snapshot";
...
export function parseJsonCliArguments(args: string[]): ParsedJsonCliCommand {
  ...
  if (command !== "list" && command !== "get" && command !== "validate" && command !== "apply" && command !== "snapshot") {
    throw new Error("Expected one of: list, get, validate, apply, snapshot");
  }
  ...
  if (command === "apply" && !patchText) throw new Error("apply requires --patch <json>");
