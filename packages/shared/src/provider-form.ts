...
/**
 * Error carrying an i18n key plus interpolation params instead of a formatted message. The code is
 * an existing locale key; the renderer catch-site resolves it with `t(error.code, error.params)`,
 * and the TUI maps codes to its own strings. Keeps shared form logic free of any i18n runtime.
 */
...
export function parseHeaders(raw: string): Record<string, string> | undefined {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new SharedError("models.headersMustBeObject");
  return parsed as Record<string, string>;
}

export function parseObjectJson(labelKey: string, raw: string): Record<string, unknown> | undefined {
  if (!raw.trim()) return undefined;
  const parsed = JSON.parse(raw);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new SharedError("models.mustBeObject", { label: labelKey });
...
export function parseModelOverrides(raw: string): Record<string, Record<string, unknown>> | undefined {
  if (!raw.trim()) return undefined;
  const parsed = JSON.parse(raw);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new SharedError("models.modelOverridesMustBeObject");
...
export function parseCost(raw: string): Record<string, number | Record<string, number>> | undefined {
  if (!raw.trim()) return undefined;
  const parsed = JSON.parse(raw);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new SharedError("models.costMustBeObject");
...
export function parseOptionalPositiveInteger(labelKey: string, value: string): number | undefined {
  if (!value.trim()) return undefined;
  const parsed = Number(value);
  if (!Number.isSafeInteger(parsed) || parsed <= 0) throw new SharedError("models.positiveIntRequired", { label: labelKey });
  return parsed;
}
...
export function buildModels(entries: ModelEditorEntry[]): OmpModel[] {
  return entries.map((entry, index) => {
    ...
    if (!id) throw new SharedError("models.modelIdEmpty", { index: index + 1 });
    const model: OmpModel = { ...entry.raw, id };
    const name = entry.name.trim();
    if (name) model.name = name;
    else delete model.name;
    const contextWindow = parseOptionalPositiveInteger("models.modelField", entry.contextWindow);
    if (contextWindow === undefined) delete model.contextWindow;
    else model.contextWindow = contextWindow;
    const maxTokens = parseOptionalPositiveInteger("models.modelField", entry.maxTokens);
    ...
    const compat = parseObjectJson("models.modelField", entry.compat);
    ...
    const remoteCompaction = parseObjectJson("models.modelField", entry.remoteCompaction);
